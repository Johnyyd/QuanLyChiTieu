...
flowchart LR
    User([Người dùng])
    Sync([Hệ thống đồng bộ])

    UC1(UC-01: Đăng nhập / Đăng ký)
    UC2(UC-02: Thêm mới giao dịch)
    UC3(UC-03: Trợ lý ảo Chatbot AI)
    UC4(UC-04: Quản lý nhóm chi tiêu)
    UC5(UC-05: Quản lý mục tiêu tiết kiệm)
    UC6(UC-06: Chỉnh sửa / Xóa giao dịch)
...
classDiagram
    class UserModel {
        ...
        + String? photoUrl
        + copyWith() UserModel
        + fromMap(Map~String, dynamic~ data, String id) UserModel$
    ...
    class ExpenseModel {
        ...
        + String currency
        + double exchangeRate
        + copyWith() ExpenseModel
        + fromMap(Map~String, dynamic~ data, String id) ExpenseModel$
        + toMap() Map~String, dynamic~
    }
    ...
    class GroupModel {
        ...
        + DateTime createdAt
        + copyWith() GroupModel
        + fromMap(Map~String, dynamic~ data, String id) GroupModel$
        + toMap() Map~String, dynamic~
    }
    ...
    class ChatMessageModel {
        ...
        + bool isUser
        + DateTime createdAt
        + fromMap(Map~String, dynamic~ data, String id) ChatMessageModel$
        + toMap() Map~String, dynamic~
    }
    ...
    class SqlServerHelper {
        ...
        + executeQuery(String query) Future~String~
        + executeWrite(String query) Future~String~
        + executeWriteWithParams(String query, Map~String, dynamic~ params) Future~String~
    }
    ...
    class AuthService {
        ...
        + signOut() Future~void~
        - syncUserToSql(User user) Future~void~
    }

    UserModel "*" <.. "1" SqlServerHelper : Fetched by
    ExpenseModel "*" <.. "1" SqlServerHelper : Saved by
    GroupModel "*" <.. "1" SqlServerHelper : Managed by
    ChatMessageModel "*" <.. "1" SqlServerHelper : Stored by
    AuthService ..> SqlServerHelper : Uses
```
